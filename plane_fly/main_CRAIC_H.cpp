...
void fly_cv_control_with_time(fly_pkg::cv_pos_srv& cv_aim_pos,ros::ServiceClient &pos_cv_client,
                            float cv_z,short cv_control_flag,float cv_times,float cv_hz,
...
void fly_vel_control(fly_pkg::vel_srv::Request& vel_pos,float pos_x,float pos_y,float pos_z,float yaw,short vel_control_flag,
                    float vel_angle_accuracy,float vel_pos_accuracy,float vel_x, float vel_y){
...
void fly_vel_control_with_time(fly_pkg::vel_srv& vel_pos,ros::ServiceClient &vel_client,float pos_x,float pos_y,float pos_z,float yaw,
    ...
    fly_vel_control(vel_pos.request, pos_x , pos_y , pos_z , yaw , vel_control_flag,vel_angle_accuracy,vel_pos_accuracy,vel_x,vel_y); 
    while(ros::ok()){         
        while(ros::ok()){
            if(vel_client.call(vel_pos))
                break;
        }
        if(vel_control_flag==Land){  //降落模式
            if(!current_state.armed){ //等待模式切换完毕
                break;
            }
        }
...
int main (int argc,char** argv){
    ...
    ros::Subscriber re_code = nh.subscribe<std_msgs::Int8>("code_sucess",10,cb_code); 
    ...
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.6 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, -1.6 , 0, 1.6 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, -1.6 , 0, 1.6 , 90*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 0.6, CV_task, 0.3f, 100, // z model time hz
                          3*PI/180 , 200);          //视觉角度精度   视觉位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, -1.6 , 0.0, 0.0 , 90*PI/180 , Land,1.0f,100, //x y z yaw model time hz
                         3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_vel_control_with_time(vel_pos,vel_client, -1.6 , 0.0, 1.6 ,0*PI/180, 0.2, 0.0 , Land,0.05f,100, //x y z yaw vel_x vel_y model vel_time vel_hz
                         3*PI/180 , 0.15f);           //vel角度精度   vel位置精度 ff 
    // /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, -1.6 , 0.0, 0.0 , 0*PI/180 , Land,1.0f,100, //x y z yaw model time hz
                         3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度 ff 
