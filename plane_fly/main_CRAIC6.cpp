...
int main (int argc,char** argv){
    ...
    ros::Subscriber sub_pose = nh.subscribe<fly_pkg::pose>("pose",10,pose_cb);
    ...
    //起飞
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.4 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                          3*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/

    //A面 上 
    fly_control_with_time(aim_pos,pos_client, 0.25 , 0.75, 1.4 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.25, 1.25, 1.4 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.25 , 1.75, 1.4 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  

    //A面 下
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.25 , 1.75, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.25 , 1.25, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/        
    fly_control_with_time(aim_pos,pos_client, 0.25 , 0.75, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/

    //A面-B面        
    fly_control_with_time(aim_pos,pos_client, 0.25 , -0.25, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.25 ,  -0.25, 1.0, 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff    
   
    //B面 下
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.25 ,  0.75, 1.0 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.25 ,  1.25, 1.0, 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.25 ,  1.75, 1.0 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    
    //B面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.25 ,  1.75, 1.4 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.25 , 1.25 , 1.4 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);     
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.25 , 0.75 , 1.4 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);    

    //C面上  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.25 , 0.75, 1.4 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , 1.25, 1.4 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , 1.75, 1.4 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);    
    
    //C面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , 1.75, 1.0 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , 1.25, 1.0 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , 0.75, 1.0 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    ...
    //C面-D面
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.25 , -0.25, 1.0 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , -0.25, 1.0 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    
    
    //D面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 0.75, 1.0 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 1.25, 1.0 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 1.75, 1.0 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    ...
    //D面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 1.75, 1.4 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 1.25, 1.4 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.25 , 0.75, 1.4 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
