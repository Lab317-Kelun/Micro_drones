...
def deal_image(img):
    ...
    ret, thresh = cv2.threshold(cv2.cvtColor(img.copy(), cv2.COLOR_BGR2GRAY), 140, 255, cv2.THRESH_BINARY)
        #ret, thresh = cv2.threshold(cv2.cvtColor(img.copy(), cv2.COLOR_BGR2GRAY), 150, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    cv2.imshow("threshold_kai_bi", closed)
    cv2.waitKey(0)
...
def one_image_demo():
    ...
    ###
    #img = cv2.imread(img)
    img = '/home/nvidia/robot_ws/src/robot_cv/script/111.png'
