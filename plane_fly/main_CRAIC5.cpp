...
int main (int argc,char** argv){
    ...
    ros::Publisher pub_code = nh.advertise<std_msgs::Int8>("re_code",10);
    ...
    //起飞
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.45 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                          3*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0 , 1.8, 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/  

    //开始识别二维码
    code.data = 1;
    pub_code.publish(code);
    //等待二维码识别完毕
    while(ros::ok()){
       ROS_INFO("code_flag: %d", code_flag.data);
       if(code_flag.data == 1){
           down = -1.6;
           break;
       }
       else if(code_flag.data == 2){
           down = 1.6;
           break;
       }
    ...
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 1.8, 1.45 , 0*PI/180 , ON,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff

    //4号点投放
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 ,  1.8, 0.6, 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 0.6, CV_task_down, yolo, 0.2f, 100, // z model vision cv_time cv_hz
                        3*PI/180 , 30);          //视觉角度精度   视觉像素精度 ff 
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_vel_control_with_time(vel_pos,vel_client, 0.0 , 0.0, 0.12 ,0*PI/180, 0.0, 0.0 ,0.0, Vel_down,0.1f,100, //x y z yaw vel_x vel_y model vel_time vel_hz
                         3*PI/180 , 0.1f);           //vel角度精度   vel位置精度 ff 
    //投放动作
    system("echo 1 > /sys/class/gpio/PN.01/value");
    system("echo 0 > /sys/class/gpio/PCC.04/value");
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 ,  1.8, 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    ...
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 3.6, 1.45 , 0*PI/180 , ON,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    //2号点投放          
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 3.6, 0.6 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 0.6, CV_task_down, yolo, 0.2f, 100, // z model vision cv_time cv_hz
                        3*PI/180 , 30);          //视觉角度精度   视觉像素精度 ff 
    ...
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.6 , 3.6, 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    system("echo 0 > /sys/class/gpio/PN.01/value");
    system("echo 0 > /sys/class/gpio/PCC.04/value");
    ...
    fly_control_with_time(aim_pos,pos_client, -1.6 , 3.6, 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, -1.0 ,  6.0, 1.45, 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff

    //特殊靶投放
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, -1.0 ,  6.0, 0.6 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 0.6, CV_task_down, yolo, 0.2f, 100, // z model vision cv_time cv_hz
                        3*PI/180 , 30);          //视觉角度精度   视觉像素精度 ff 
    ...
    //fly_control_with_time(aim_pos,pos_client, 0.0 ,  6.0, 1.45 , -90*PI/180 , ON,1.2f,100, //x y z yaw model time hz
    //                    3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff

    /*-------------------------------------------------------------------------------------------------*/
    // cv视觉锁定圆环
    //fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 1.45, CV_task_circle, cv, 0.5f, 100, // z model vision cv_time cv_hz
    //                   3*PI/180 , 30);          //视觉角度精度   视觉像素精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    // 发送速度穿越圆环
    //fly_vel_control_with_time(vel_pos,vel_client, 2.1 , 0.0, 1.45 ,-90*PI/180, 0.4, 0.0 ,0.0, Vel_circle,0.01f,100, //x y z yaw vel_x vel_y model vel_time vel_hz
    //                    3*PI/180 , 0.15f);           //vel角度精度   vel位置精度 ff 
    
    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.1 , 6.0 , 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.1 , 2.7 , 1.45 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, down , 0.0, 1.45 , 0*PI/180 , ON,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, down , 0.0, 0.6 ,  0*PI/180 , ON,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    /*-------------------------------------------------------------------------------------------------*/

    //视觉纠正降落
    fly_cv_control_with_time(aim_cv_pos,pos_cv_client, 0.6, CV_task_down, yolo, 0.5f, 100, // z model vision cv_time cv_hz
                        3*PI/180 , 30);          //视觉角度精度   视觉像素精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_vel_control_with_time(vel_pos,vel_client, 0.0 , 0.0, 0.0 ,0*PI/180, 0.0, 0.0 ,-0.8, Vel_Land,0.1f,100, //x y z yaw vel_x vel_y model vel_time vel_hz
                         3*PI/180 , 0.1f);           //vel角度精度   vel位置精度 ff 
