...
std_msgs::Int8 codes;
void light_time(float time, ros::Publisher &pub_code){
    ros::Time last_times = ros::Time::now();
    while(ros::ok()){
        //二维码识别发送 并激光射击
        codes.data = 1;
        pub_code.publish(codes);
        system("echo 1 > /sys/class/gpio/PN.01/value");
        
        if(ros::Time::now() -last_times> ros::Duration(time)){
            codes.data = 0;
            pub_code.publish(codes);
...
int main (int argc,char** argv){
    ...
    ros::Publisher pub_code = nh.advertise<std_msgs::Int8>("code",10);
    ...
    //起飞
    /*-------------------------------------------------------------------------------------------------*/  
    fly_control_with_time(aim_pos,pos_client, 0 , 0 , 1.5 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                          3*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    //fly_control_with_time(aim_pos,pos_client, 0.15 , 0 , 1.35 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
    //                      3*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/


    //A面 上 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 0.75, 1.35 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15, 1.25, 1.35 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.75, 1.35 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);  

    //A面 下
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.75, 1.0 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.25, 1.0 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/        
    fly_control_with_time(aim_pos,pos_client, 0.15 , 0.75, 1.0 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/

    //A面-B面        
    fly_control_with_time(aim_pos,pos_client, 0.15 , -0.25, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , -0.25, 1.0 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , 1.25, 1.0, 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , 1.25, 1.0, 180*PI/180 , ON,0.25f,100, //x y z yaw model time hz
                        6*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff  
   
    //B面 下
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  0.75, 1.0 , 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.25, 1.0, 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.75, 1.0 , 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);   
    
    //B面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.75, 1.35 , 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_time(0.7,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 , 1.25 , 1.35 , 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);     
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 , 0.75 , 1.35 , 180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);
    ...
    //B面-C面    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , 1.25, 1.35 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , 1.25, 1.35 , 0*PI/180 , ON,0.25f,100, //x y z yaw model time hz
                        6*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
                        
    //C面上  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.15 , 0.75, 1.35 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.15 , 1.25, 1.35 ,  0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.15 , 1.75, 1.35 ,  0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);    
    
    //C面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.15 , 1.75, 1.0 ,  0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.15 , 1.25, 1.0 ,  0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.15 , 0.75, 1.0 ,  0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    ...
    fly_control_with_time(aim_pos,pos_client, 1.75 , 1.25, 1.0 , 180*PI/180 , ON,0.25f,100, //x y z yaw model time hz
                        6*PI/180 , 0.15f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , -0.25, 1.0 , 180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , -0.25, 1.0 ,  180*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    
    
    //D面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 0.75, 1.0 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.25, 1.0 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.75, 1.0 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    ...
    //D面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.75, 1.35 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.25, 1.35 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 0.75, 1.35 ,  180*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_time(0.7,pub_code);
            

    //降落点
    fly_control_with_time(aim_pos,pos_client, 3.5 , 2.5, 1.0 ,  180*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    //降落
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.5 , 2.5, 0.0 ,  180*PI/180 , Land,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
