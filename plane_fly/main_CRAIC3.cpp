...
bool light_right_time(float time, ros::Publisher &pub_code){
    ros::Time last_times = ros::Time::now();
    while(ros::ok()){
        //二维码识别发送 并激光射击 右边
...
bool light_left_time(float time, ros::Publisher &pub_code){
    ros::Time last_times = ros::Time::now();
    while(ros::ok()){
        //二维码识别发送 并激光射击 左边
        if(ros::Time::now() -last_times> ros::Duration(time)){
...
int main (int argc,char** argv){
    ...
    //A面 上 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 0.75, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                          3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15, 1.25, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.75, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);   

    //A面 下
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.75, 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);     
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 0.15 , 1.25, 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/        
    fly_control_with_time(aim_pos,pos_client, 0.15 , 0.75, 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_right_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/

    //A面-C面        
    fly_control_with_time(aim_pos,pos_client, 0.15 , -0.25, 0.85 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , -0.25, 0.85 , 0*PI/180 , ON,0.75f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 

   
    //C面 下  
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 2.05 , 0.75, 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.05 , 1.25, 0.85 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.05 , 1.75, 0.85 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);      
    ...
    //C面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.05 , 1.75, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.05 , 1.25, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);     
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 2.05 , 0.75, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_right_time(0.5,pub_code);     
    ...
    //B面 上
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  0.75, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_left_time(0.5,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.25, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_left_time(0.5,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.75, 1.25 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_left_time(0.5,pub_code);   
    
    //B面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 ,  1.75, 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff
    light_left_time(0.5,pub_code);     
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.35 , 1.25 , 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);      
    /*-------------------------------------------------------------------------------------------------*/ 
    fly_control_with_time(aim_pos,pos_client, 1.35 , 0.75 , 0.85 , 0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code); 
    ...
    //B面-D面    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 1.75 , -0.25, 0.85 , 0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 25.0f);          //角度精度   位置精度 ff 
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , -0.25, 0.85 ,  0*PI/180 , ON,0.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    
    
    //D面 下
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 0.75, 0.85 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.25, 0.85 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);   
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.75, 0.85 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);    
    ...
    //D面 上
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.75, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 1.25, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code);    
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.35 , 0.75, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    light_left_time(0.5,pub_code); 
            

    //降落点
    fly_control_with_time(aim_pos,pos_client, 3.5 , 2.5, 1.25 ,  0*PI/180 , ON,1.0f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    //降落
    /*-------------------------------------------------------------------------------------------------*/
    fly_control_with_time(aim_pos,pos_client, 3.5 , 2.5, 0.0 ,  0*PI/180 , Land,1.5f,100, //x y z yaw model time hz
                        3*PI/180 , 0.1f, 3*PI/180, 25.0f, 15.0f);           //角度精度   位置精度
    ...
    system("rosnode kill t265_to_mavros");
    system("rosnode kill realsense2_camera");
    system("rosnode kill realsense2_camera_manager");
    system("rosnode kill pos_CRAIC");
